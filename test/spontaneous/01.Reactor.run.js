class Reactor {
  ...
  _validatePostevents(postevents, sourcevent) {
    for(let index=0; index<postevents.length; index++) {
      ...
      this.assert(["string","function"].includes(typeof postevent), `Reactor event «${sourcevent}» at argument position of «${index}» has a type «${typeof postevent}» but only string or function are allowed here`);
    }
  }
  ...
  trigger(event, parameters = {}) {
    ...
    return Promise.all(this.listeners[event].map((selector, index) => {
      ...
      this.assert(typeof listener === "function", `Reactor.prototype.trigger found that event «${event}» at index «${index}» has a selector of type «${typeof selector}» but only function or string are allowed selectors`, () => console.log(selector));
      listener(parameters);
    }));
  }
  _noop() {}
