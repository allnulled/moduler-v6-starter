(() => {
  ...
  const RunnableRace = {
    startRace(instructions, injection = false) {
      if (!Array.isArray(instructions)) throw new Error("Parameter «instructions» must be array on «Runnable.prototype.startRace»");
      return Promise.race(instructions.map((step, index) => {
        if (typeof step === "string") this[step](...(injection || []));
        else if (typeof step === "function") step.call(this, ...(injection || []));
        else throw new Error(`Parameter «instructions[${index}]» must be string or function on «Runnable.prototype.startRace»`);
      }));
    }
  };
  ...
  Cycle = class Cycle {
    static {
      ...
      Object.defineProperties(this, Object.getOwnPropertyDescriptors(Newable));
    }
    static all = {};
    static find(id) {
      return this.all[id];
  ...
  console.log(Cycle.new.as("Ciclo inicial").config({ name: "alt mainSync" }).startSync(["mainSync"]));
  Cycle.new.as(Cycle.find("Ciclo inicial")).config({ name: "in sync" }).startSync(["main"]);
  Cycle.new.as("Ciclo inicial").config({ name: "in parallel" }).startParallel(["main"]);
