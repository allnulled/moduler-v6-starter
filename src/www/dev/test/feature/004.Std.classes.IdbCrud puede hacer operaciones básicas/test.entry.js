module.exports = async function({ Tester }) {
  ...
  Tester.start("Tests of Std.classes.IdbFilesystem", async function({ tester, asserter: { assert } }) {
    let fs;
    ...
    await tester.case("can do deleteFile and hasFile", async function() {
      assert(true === await fs.hasFile("./hello.txt"), "hasFile works with recently created file");
      await fs.deleteFile("./hello.txt");
      assert(false === await fs.hasFile("./hello.txt"), "hasFile works with recently deleted file");
    });
    ...
    await tester.case("can do deleteDirectory and hasDirectory", async function() {
      assert(true === await fs.hasDirectory("./lib"), "hasDirectory works with recently created directory");
      await fs.deleteDirectory("./lib");
      assert(false === await fs.hasDirectory("./lib"), "hasDirectory works with recently deleted directory");
    });
