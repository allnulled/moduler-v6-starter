  ...
  return function(mod) {
    ...
    const CompilerV6 = class CompilerV6 {
      ...
      static Tracer=class {
        ...
        traceOut(msg, args) {
          this.stack[this.stack.length - 1], this.stack.pop(), this.trace(msg, args, -1);
        }
        printStack() {
      ...
      static ansi={
        colors: Object.assign({
          ...
          style: function(config = "red,bold,underline") {
            ...
            return {
              text: text => `${styles.reduce((out, it) => it in this.available ? out += `[${this.available[it][0]}m` : out, "")}${text}${this.endToken}`,
              print(text) {
          ...
          padLinesToMax: function(text) {
            ...
            for (let index = 0; index < lines.length; index++) 0 !== index && (out += "\n"), 
      ...
      async _compileAsInjectSource(compilationFile, compilationProcess, {token: token, tokenIndex: tokenIndex}, options = {}) {
        ...
        try {
          ...
          Compile_target: {
            ...
            if (!(currentStep.push("4.b. compiled target newly"), compilationProcess.dontCreateOnInjectSource || await this._existsFile(targetPath))) {
              currentStep.push("4.b.1. create injected file as it does not exist"), require("path");
          ...
          if (currentStep.push("5. inject text in compilation"), "html" === compilationFile.extension) {
      ...
      async _compileAsInjects(compilationFile, compilationProcess, {token: token, tokenIndex: tokenIndex}) {
        ...
        });
        {
          ...
          if (fromJs && parameters[0].endsWith(".js")) return await this._compileAsInjectSource(compilationFile, compilationProcess, {
            token: token,
            tokenIndex: tokenIndex
          });
        }
      ...
      _reportFileToken(compilationFile, targetBrute, token) {
        ...
        this.rootdirOf(targetBrute), owner in compilationFile.report.tree || (compilationFile.report.tree[owner] = {});
    ...
    static Cronometer=() => {
      ...
      const getTask = function(name) {
        return tasks[name] || (tasks[name] = {
          ...
          open(label) {
            const now = new Date;
            return this.openedAt = now, this.lastMarkAt = now, this.stoppedAt = null, this.marks = [], 
            this.order = tasks.counter++, label && this.mark(label), this;
          },
          mark(label) {
            const now = new Date;
            return this.marks.push({
              label: label,
              fromLast: now - this.lastMarkAt,
              fromStart: now - this.openedAt
            }), this.lastMarkAt = now, this;
          },
          stop(label) {
            return label && this.mark(label), this.stoppedAt = new Date, this;
          },
          milliseconds() {
      ...
      return getTask.pick = function(name, defaultValue = null) {
    ...
    static Utils=class {
      ...
      static async findFirstParentDirectoryContaining(dirBrute, file = "package.json", includingSelf = !0) {
        const fs = require("fs").promises, path = require("path"), dir = path.resolve(dirBrute);
        let prevDir2, dir2 = includingSelf ? dir : path.dirname(dir), selectedDir = !1;
        Search_directory_up: for (;dir2 !== prevDir2; ) {
          const filepath = path.resolve(dir2, file);
          try {
            await fs.readFile(filepath, "utf8"), selectedDir = dir2;
            break Search_directory_up;
          } catch (error) {}
          prevDir2 = dir2, dir2 = path.dirname(dir2);
        }
      ...
      static removeNullPropertiesFromObject(obj) {
        ...
        for (let prop in obj) {
          const val = obj[prop];
          null !== val ? output[prop] = val : console.log("Removed: " + prop, val);
        }
        return output;
      }
      assert(...args) {
      ...
      parseCliArgs(args) {
        this.assert("object" == typeof args, "Parameter «args» must be object on «DevBinaryV6.Utils.prototype.parseCliArgs»"), 
        this.assert(Array.isArray(args), "Parameter «args» must be array on «DevBinaryV6.Utils.prototype.parseCliArgs»"), 
        this.assert(0 !== args.length, "Parameter «args» must have at least 1 item on «DevBinaryV6.Utils.prototype.parseCliArgs»");
        let params = {
          _: []
        }, selected = "_";
        for (let index = 0; index < args.length; index++) {
          const arg = args[index];
          arg.startsWith("-") ? (selected = arg, params[selected] = params[selected] || []) : params[selected].push(arg);
        }
        return params;
      }
      formatCliArgs(definition = !1, argsBrute = process.argv) {
        let args, result, usedKeys;
        this.assert("object" == typeof definition, "Parameter «definition» must be object on «DevBinaryV6.Utils.prototype.formatCliArgs»"), 
        this.assert("object" == typeof argsBrute, "Parameter «args» must be object on «DevBinary.Utils.prototype.formatCliArgs»"), 
        this.assert(null !== argsBrute, "Parameter «args» cannot be null on «DevBinary.Utils.prototype.formatCliArgs»"), 
        args = Array.isArray(argsBrute) ? this.parseCliArgs(argsBrute) : argsBrute, result = {}, 
        result._ = args ? args._ : [], usedKeys = new Set([ "_" ]);
        Iterating_definition_entries: for (const [name, config] of Object.entries(definition)) {
          const longKey = "--" + name, aliases = config.alias || [], sources = [];
          longKey in args && sources.push({
            key: longKey,
            value: args[longKey]
          });
          for (const alias of aliases) alias in args && sources.push({
            key: alias,
            value: args[alias]
          });
          if (sources.length > 1) throw new Error(`Option "${name}" was specified multiple times (${sources.map(v => v.key).join(", ")}).`);
          if (0 === sources.length) {
            "default" in config && (result[name] = config.default);
            continue Iterating_definition_entries;
          }
          usedKeys.add(longKey);
          for (const alias of aliases) usedKeys.add(alias);
          let value = sources[0].value;
          "function" == typeof config.onFormat && (value = config.onFormat.call(this, [ ...value ])), 
          result[name] = value;
        }
        for (const key of Object.keys(args)) if (!usedKeys.has(key)) {
      ...
      async compileDistribuiblesOf(filepath, event) {
        ...
        const currentStep = [];
        try {
          currentStep.push("1. begin"), report = {}, currentStep.push("2. compile"), compilation = await this.devbin.compiler.compile(filepath, {
            processedEntries: event.processedEntries,
            uncacheInjections: event.uncacheInjections,
            dontCreateOnInjectSource: !1
          });
          {
            currentStep.push("3. compose output paths");
            const outputNames = this.getDistribuibleFilenamesOf(compilation.file, event), inputDir = require("path").dirname(outputNames.file), inputRootdir = this.devbin.compiler.rootdirOf(inputDir);
            let outputDir;
            outputDir = event.isSrcWww ? this.devbin.compiler.fullpathOf(inputRootdir.replace(/^\@\/src\/www/g, "@/dist/www")) : this.devbin.compiler.fullpathOf(inputRootdir.replace(/^\@\//g, "@/dist/")), 
            distJs = require("path").resolve(outputDir, outputNames.js), distCss = require("path").resolve(outputDir, outputNames.css), 
            distMd = require("path").resolve(outputDir, outputNames.md), srcDistJs = require("path").resolve(inputDir, outputNames.js), 
            srcDistMd = require("path").resolve(inputDir, outputNames.md), report.names = outputNames;
          }
          if (currentStep.push("4. safety assertions 1"), this.assert(distJs.endsWith(".dist.js"), `File should end with «.dist.js» but it is not the case on «${distJs}»`), 
          ...
          currentStep.push("5. ensure output directory"), await this.ensureDirectoryOf(distJs), 
          compilation.js) {
            let output;
            currentStep.push("6. minify"), output = await this.devbin.compiler.constructor.softMinifyJs(compilation.js, {
              compress: !1,
              mangle: !1,
              toplevel: !0,
              format: {
            ...
            }), currentStep.push("7. write js file"), await require("fs").promises.writeFile(distJs, output.code, "utf8"), 
            ...
            report.js = distJs, event.processedEntries[compilation.file] = {
              distJs: distJs
            };
            {
              currentStep.push("7. load settings"), await this.devbin.settings.load();
              const instrumentalizeFiles = this.devbin.settings?.data?.instrumentalize || [];
              if (instrumentalizeFiles.map(file => this.devbin.moduler.normalizationOf(file)).includes(distJs)) {
                currentStep.push("8. generate instrumentalization");
                {
                  const instrJs = distJs.replace(/\.dist\.js$/g, ".dist.instr.js"), instrSource = this.instrumentCode(output.code, distJs);
                  await require("fs").promises.writeFile(instrJs, instrSource, "utf8"), console.log(this.devbin.compiler.constructor.ansi.colors.style("blackBright").text(`[*] DevBinaryV6 generated instrumentation file at: ${this.devbin.compiler.rootdirOf(instrJs)}`));
                }
              }
            }
          }
          return compilation.css && (currentStep.push("9. write css file"), await require("fs").promises.writeFile(distCss, compilation.css, "utf8"), 
          event.processedEntries[compilation.file] || (event.processedEntries[compilation.file] = {}), 
          event.processedEntries[compilation.file].distCss = distCss, report.css = distCss), 
          compilation.md && (currentStep.push("10. write md file"), await require("fs").promises.writeFile(distMd, compilation.md, "utf8"), 
          event.processedEntries[compilation.file] || (event.processedEntries[compilation.file] = {}), 
          event.processedEntries[compilation.file].distMd = distMd, report.md = distMd), currentStep.push("last. return report"), 
          report;
        } catch (error) {
          throw console.log(`[!] Error on method «compileDistribuiblesOf» on step «${currentStep.reverse().join(" < ")}»`, error), 
          error;
        }
      }
      getDistribuibleFilenamesOf(fileBrute, event) {
        let file, filename, fileExtension;
        if (file = require("path").basename(fileBrute), file.endsWith(".entry.js")) filename = file.substr(0, file.length - 9), 
        fileExtension = "js"; else if (file.endsWith(".entry.css")) filename = file.substr(0, file.length - 10), 
        fileExtension = "css"; else {
          if (!file.endsWith(".entry.md")) throw new Error(`Parameter «file» must end with «.entry.js», «.entry.css» or «.entry.md» but it is «${file}» on «DevBinaryV6.Utils.prototype.getDistribuibleFilenamesOf»`);
          filename = file.substr(0, file.length - 9), fileExtension = "md";
        }
        return {
          file: fileBrute,
      ...
      async fabricateUnitTestFileOf(filepath, event) {
        if (event.isSrcWww) return -2;
        if (!event.distribution.js) return -3;
        const path = require("path"), fs = require("fs"), testunitFile = path.resolve(event.distribution.names.rootdirDirectory.replace(/^\@\/src/g, this.devbin.compiler.fullpathOf("@/test/unit/src")), event.distribution.names.test), devBinaryV6Filepath = this.devbin.compiler.fullpathOf("@/dev/bin.js"), devBinaryV6RelativeFilepath = path.relative(path.dirname(testunitFile), devBinaryV6Filepath), relativeTarget = path.relative(path.dirname(testunitFile), event.distribution.js), testunitContent = `const devbin = require(__dirname + ${JSON.stringify("/" + devBinaryV6RelativeFilepath)});\nconst target = require(__dirname + ${JSON.stringify("/" + relativeTarget)});\n\nmodule.exports = (async function () {\n\n  devbin.assert(true, "Test is empty right now");\n\n})();`, testunitDir = path.dirname(testunitFile);
        return await this.existsFile(testunitFile) || (await fs.promises.mkdir(testunitDir, {
          recursive: !0
      ...
      executeUnitTestFileOf(filepath, event) {
        if (!event.isSrcWww) return delete require.cache[event.testFabrication.unitFile], 
        event.testFabrication.unitFile ? require(event.testFabrication.unitFile) : -2;
        console.log(`[*] DevBinaryV6 ignored test for browser file: ${filepath}`);
      }
      async propagateUpTouchEventFrom(filepath, event = {}) {
        const fs = require("fs"), path = require("path");
        let pivotDirectory, firstFile, nextPropagationFiles = [], currentDirectory = path.dirname(path.resolve(filepath)), currentDirectoryName = path.basename(currentDirectory), upperDirectory = path.dirname(currentDirectory);
        {
          const possibleMainEntry = `${currentDirectory}/${currentDirectoryName}.entry.js`;
          await this.existsFile(possibleMainEntry) && await this.touchFile(possibleMainEntry, {
            propagateUp: !1,
            processedEntries: event.processedEntries || {}
          });
        }
        pivotDirectory = upperDirectory;
        Iterating_entries: for (;;) {
          const matchedEntries = (await fs.promises.readdir(pivotDirectory, {
            withFileTypes: !0
          })).filter(e => e.isFile() && (e.name.endsWith(".entry.js") || e.name.endsWith(".entry.css") || e.name.endsWith(".entry.md"))).map(e => path.resolve(e.path, e.name));
          if (matchedEntries.length) {
            nextPropagationFiles = matchedEntries;
            break Iterating_entries;
          }
          const parentDir = path.dirname(pivotDirectory);
          if (parentDir === pivotDirectory) return null;
          pivotDirectory = parentDir;
        }
        return firstFile = nextPropagationFiles[0], await Promise.all(nextPropagationFiles.map(file => this.touchFile(file, {
          propagateUp: !1,
          processedEntries: event.processedEntries || {}
        }))), this.propagateUpTouchEventFrom(firstFile, event);
      }
      ...
      async touchFile(file, optionsInput = {}) {
        this.assert("string" == typeof file, `Parameter «--file» must be string and not «${typeof file}» on «DevBinaryV6.Utils.prototype.touchFile»`);
        const currentStep = [];
        try {
          let fs, path, filepath, rootPath, event, isEntry, outputFile = !1;
          currentStep.push("0. begin with: " + file), currentStep.push("1. initialize dependencies"), 
          fs = require("fs"), path = require("path"), filepath = this.devbin.compiler.fullpathOf(file), 
          rootPath = this.devbin.moduler.rootdirOf(filepath), currentStep.push("1. initialize event for: " + rootPath), 
          event = this.constructor.defaultTouchFileOptions({
            type: "TouchFileEvent",
            propagateUp: !0,
            ignoreOnTouchEvent: !1,
            processedEntries: {},
            isRoot: !1,
            ...optionsInput
          }), this.assert(optionsInput.uncacheInjections === event.uncacheInjections, "Las inyections 2"), 
          event.isHtml = filepath.endsWith(".html"), event.isJsEntry = filepath.endsWith(".entry.js"), 
          event.isCssEntry = filepath.endsWith(".entry.css"), event.isMdEntry = filepath.endsWith(".entry.md"), 
          event.isJsTest = filepath.endsWith(".test.js"), event.isSrcWww = rootPath.startsWith("@/src/www/"), 
          event.isSrc = rootPath.startsWith("@/src/"), isEntry = event.isJsEntry || event.isCssEntry || event.isMdEntry;
          Touch_event: if (currentStep.push("3. run touch event"), filepath !== this.devbin.compiler.fullpathOf("@/dev/settings.js")) {
            if (event.isHtml) {
              if (currentStep.push("3.2. found html file"), event.isSrcWww) currentStep.push("3.2.a. html is src/www/**/*.html"), 
              outputFile = `@/dist/www/${rootPath.replace("@/src/www/", "")}`; else {
                if (!event.isSrc) {
                  currentStep.push("3.2.c. html is not src/**/*.html"), console.log(this.devbin.compiler.constructor.ansi.colors.style("blackBright").text(`[-] DevBinaryV6 dismissed touch event from an *.html not under «@/src/»: ${rootPath}`));
                  break Touch_event;
                }
                currentStep.push("3.2.b. html is src/**/*.html"), outputFile = `@/dist/src/${rootPath.replace("@/src/", "")}`;
              }
              currentStep.push("3.2.{a,b}. compiling html file");
              const outputHtml = (await this.devbin.compiler.compile(filepath)).html, outputFullpath = this.devbin.moduler.normalizationOf(outputFile);
              await require("fs").promises.writeFile(outputFullpath, outputHtml, "utf8");
            }
            if (isEntry || event.isJsTest) {
              currentStep.push("3.3.b. is entry or test"), console.log(this.devbin.compiler.constructor.ansi.colors.style("blackBright").text(`[*] DevBinaryV6 triggered touch event from: ${rootPath}`)), 
              currentStep.push("3.4. compile distribuibles of entry "), Object.assign(event, {
                distribution: await this.compileDistribuiblesOf(filepath, event)
              }), currentStep.push("3.5. make unit test"), Object.assign(event, {
                testFabrication: await this.fabricateUnitTestFileOf(filepath, event)
              }), currentStep.push("3.6. run unit test"), Object.assign(event, {
              ...
              }
              {
                ...
                const onTestFile = path.join(path.dirname(filepath), "e.onTest.js"), testsAdded = await this.triggerCallbackFromFile(onTestFile, {
                  file: filepath,
                  event: event
                });
                "number" != typeof testsAdded && (this.assert("object" == typeof testsAdded, `File «e.onTest.js» must return object about file «${onTestFile}» on «DevBinaryV6.Utils.prototype.touchFile»`), 
                ...
                }), "feature" in testsAdded && event.testFeatures.push(...testsAdded.feature), "integrity" in testsAdded && event.testIntegrity.push(...testsAdded.integrity), 
                "speed" in testsAdded && event.testSpeed.push(...testsAdded.speed));
              }
            } else currentStep.push("3.3.a. is not entry nor test"), console.log(this.devbin.compiler.constructor.ansi.colors.style("blackBright").text(`[-] DevBinaryV6 dismissed touch event from not entry or test: ${rootPath}`));
            if (event.isJsTest) currentStep.push("4. run file because it is a test"), await this.executeUnitTestFileOf(filepath, {
              testFabrication: {
                unitFile: filepath
              }
            }); else {
              ...
              }
              {
                ...
                if (outputFile && !0 === result) {
                  currentStep.push("6.1. distributing directory");
                  const origin = path.dirname(this.devbin.compiler.normalizationOf(rootPath)), destination = path.dirname(this.devbin.compiler.normalizationOf(outputFile));
              ...
              }
              {
                ...
                const touchPropagation = !!event.propagateUp && await this.propagateUpTouchEventFrom(filepath, event);
                Object.assign(event, {
                  touchPropagation: touchPropagation
                });
              }
              if (event.isRoot) {
                currentStep.push("7. it is root"), currentStep.push("7.1. run integrity tests"), 
                ...
                }), currentStep.push("7.2. run speed tests"), await this.devbin.tester.runDirectory("@/test/speed", {
                  title: "speed",
                  filename: "speed.js",
                  filter: file => this.matchesFileWithSimpleSelector(path.basename(file), [ ...event.testSpeed, ...this.devbin.settings.data?.test?.speed || [] ])
                }), currentStep.push("7.3. run feature tests"), await this.devbin.tester.runDirectory("@/test/feature", {
                  title: "feature",
                  filename: "feature.js",
                ...
                }), currentStep.push("7.3. run case tests"), await this.devbin.tester.runDirectory("@/test/case", {
                  title: "case",
                  filename: "case.js",
                  filter: file => !0
                });
          ...
          } else currentStep.push("3.1. exporting dev/settings"), await this.exportDevSettings(filepath);
          return event;
        } catch (error) {
          throw console.log(`[!] Error on method «touchFile» on step «${currentStep.reverse().join(" < ")}»`, error), 
          error;
        }
      }
      async ensureCoreFrom(basedirInput, parametersInput = {}) {
        this.devbin.compiler.normalizationOf(basedirInput);
        const parameters = Object.assign({}, {
          ...
          from: basedirInput
        }), fs = require("fs"), targetDir = require("path").resolve(parameters.from), innerFiles = await fs.promises.readdir(targetDir);
        parameters.allowDirtyDirectory || this.assert(0 === innerFiles.length, `Parameter «--from» should point to an empty directory but «${targetDir}» is not empty on «DevBinaryV6.Utils.prototype.ensureCoreFrom»`);
        ...
        Object.assign(utils, {
          ...
          _saveFile: async function(file, contents) {
            return parameters.dontOverride && await utils._existsFile(file) ? -1 : await fs.promises.writeFile(file, contents, "utf8");
          },
          _saveFileIfNotExists: async function(file, contents) {
            return await utils._existsFile(file) ? -1 : await fs.promises.writeFile(file, contents, "utf8");
          },
          _duplicateFile: async function(src, dst) {
            return parameters.dontOverride && await utils._existsFile(dst) ? -1 : await fs.promises.copyFile(src, dst);
          },
          ...
          _initializeDuplicatedFile: async function(src, dst) {
            if (!await utils._existsFile(dst)) return await fs.promises.copyFile(src, dst);
          },
          _readFile: function(src) {
            return fs.promises.readFile(src, "utf8");
          },
          trify: function(callback, errorSignal = !1) {
          ...
          _existsFile: utils.trify(utils._readFile, !1)
        }), parameters.ignoreErrors ? utils.trify(utils._createDirectory) : utils._createDirectory;
        const createDirectoryIfNotExists = utils.trify(utils._createDirectory), saveFile = parameters.ignoreErrors ? utils.trify(utils._saveFile) : utils._saveFile, saveFileIfNotExists = utils._saveFileIfNotExists, duplicateFile = parameters.ignoreErrors ? utils.trify(utils._duplicateFile) : utils._duplicateFile, duplicateDirectory = parameters.ignoreErrors ? utils.trify(utils._duplicateDirectory) : utils._duplicateDirectory, duplicateFileIfNotExists = utils.trify(utils._initializeDuplicatedFile);
        return await createDirectoryIfNotExists(`${targetDir}/dev`), await createDirectoryIfNotExists(`${targetDir}/dev/bin`), 
        await createDirectoryIfNotExists(`${targetDir}/dev/bin/help`), await createDirectoryIfNotExists(`${targetDir}/dev/bin/test`), 
        await createDirectoryIfNotExists(`${targetDir}/dev/coverage`), await createDirectoryIfNotExists(`${targetDir}/dev/files`), 
        await createDirectoryIfNotExists(`${targetDir}/src`), await createDirectoryIfNotExists(`${targetDir}/src/external`), 
        await createDirectoryIfNotExists(`${targetDir}/src/www`), await createDirectoryIfNotExists(`${targetDir}/src/www/dev`), 
        await createDirectoryIfNotExists(`${targetDir}/src/www/external`), await createDirectoryIfNotExists(`${targetDir}/dist`), 
        await createDirectoryIfNotExists(`${targetDir}/dist/src`), await createDirectoryIfNotExists(`${targetDir}/dist/www`), 
        await createDirectoryIfNotExists(`${targetDir}/dist/www/coverage`), await createDirectoryIfNotExists(`${targetDir}/dist/www/external`), 
        await createDirectoryIfNotExists(`${targetDir}/dist/www/dev`), await createDirectoryIfNotExists(`${targetDir}/dist/www/dev/settings`), 
        await createDirectoryIfNotExists(`${targetDir}/dist/src/external`), await createDirectoryIfNotExists(`${targetDir}/test`), 
        await createDirectoryIfNotExists(`${targetDir}/test/feature`), await createDirectoryIfNotExists(`${targetDir}/test/integrity`), 
        await createDirectoryIfNotExists(`${targetDir}/test/unit`), await createDirectoryIfNotExists(`${targetDir}/test/unit/src`), 
        await createDirectoryIfNotExists(`${targetDir}/test/case`), await createDirectoryIfNotExists(`${targetDir}/test/speed`), 
        await createDirectoryIfNotExists(`${targetDir}/docs`), await createDirectoryIfNotExists(`${targetDir}/docs/dist`), 
        await createDirectoryIfNotExists(`${targetDir}/docs/dist/www`), await createDirectoryIfNotExists(`${targetDir}/docs/dist/www/external`), 
        await saveFileIfNotExists(`${targetDir}/package.json`, JSON.stringify(initialPackageJson, null, 2), "utf8"), 
        await utils._existsFile(`${targetDir}/.gitignore`) || await saveFile(`${targetDir}/.gitignore`, "node_modules", "utf8"), 
        ...
        await fs.promises.chmod(`${targetDir}/dev/run.js`, "755"), await duplicateFileIfNotExists(`${__dirname}/../src/DevBinaryV6/Utils/core/index.html`, `${targetDir}/src/www/index.html`), 
        ...
        await duplicateDirectory(`${__dirname}/refrescador`, `${targetDir}/src/external/refrescador`, {
          recursive: !0
        }), parameters.installDependencies && await this.installNpmDependencies([], targetDir), 
        {
          targetDir: targetDir
        };
      }
      ...
      async triggerCallbackFromFile(file, injection = {}, dontThrow = !1) {
        if (!await this.existsFile(file)) return -1;
        const callback = require(file);
        return this.assert("function" == typeof callback, `File «${file}» should export a function on «DevBinaryV6.Utils.prototype.triggerCallbackFromFile»`), 
      ...
      async exportDevSettings(filepath) {
        try {
          const fs = require("fs"), settingsAsyncFactory = require(filepath), settingsData = "function" == typeof settingsAsyncFactory ? await settingsAsyncFactory({
            devbin: this.devbin
          }) : settingsAsyncFactory, publicableSettingsData = {};
          ...
          const publicableSettings = this.constructor.removeNullPropertiesFromObject(publicableSettingsData), publicableJson = this.devbin.compiler.fullpathOf("@/dist/www/dev/settings/publicable.json");
          await this.ensureDirectoryOf(publicableJson), await fs.promises.writeFile(publicableJson, JSON.stringify(publicableSettings, null, 2), "utf8");
        } catch (error) {
      ...
      matchesFileWithSimpleSelector(filepath, selectors = []) {
        return this.assert(Array.isArray(selectors), "Parameter «selectors» must be array on «DevBinaryV6.Utils.prototype.matchesFileWithSimpleSelector»"), 
        selectors.some((selector, index) => (this.assert("string" == typeof selector, `All selectors must be strings but on index «${index}» there is a «${typeof selector}»`), 
        selector.startsWith("^") ? filepath.startsWith(selector.slice(1)) : filepath.includes(selector)));
      }
      ...
      async installNpmDependencies(files, rootdir = this.devbin.moduler.rootdir) {
        const {exec: exec} = require("child_process"), {promisify: promisify} = require("util"), execAsync = promisify(exec), command = "npm install" + (files ? ` ${files.join(" ")}` : ""), {stdout: stdout, stderr: stderr} = await execAsync(command, {
          cwd: rootdir
        });
    ...
    static ShadowCommands=class {
      ...
      "new project"(args, devbin) {
        ...
        return this.assert("string" == typeof parameters.from, "Parameter «--from» is required as string on «DevBinaryV6.ShadowCommands.prototype['new project']»"), 
        devbin.utils.ensureCoreFrom(parameters.from, {
          ignoreErrors: 0,
      ...
      async "ensure core"(args, devbin) {
        ...
        return this.assert("string" == typeof parameters.from, "Parameter «--from» is required as string on «DevBinaryV6.ShadowCommands.prototype['ensure core']»"), 
        this.assert("boolean" == typeof parameters.reset, "Parameter «--reset» is required as boolean on «DevBinaryV6.ShadowCommands.prototype['ensure core']»"), 
        devbin.utils.ensureCoreFrom(parameters.from, {
          ignoreErrors: 1,
          allowDirtyDirectory: 1,
      ...
      async "build github pages"(args, devbin) {
        await devbin.compiler.files.copyDirectory("@/dist/www", "@/docs/dist/www"), await devbin.compiler.files.copyFile.try("@/dist/www/index.html", "@/docs/index.html"), 
      ...
      async loop(args) {
        ...
        const port = this.devbin.settings.data?.loop?.port || 3005, settingsControllers = this.devbin.settings.data?.loop?.controllers || [], targetDirs = [ require("path").resolve(targetRoot, "src"), require("path").resolve(targetRoot, "dev/settings.js"), require("path").resolve(targetRoot, "test/unit/src"), require("path").resolve(targetRoot, "test/feature"), require("path").resolve(targetRoot, "test/integrity"), require("path").resolve(targetRoot, "test/spontaneous") ], devControllersFile = `${targetRoot}/dev/controllers.js`, devControllers = await this.devbin.utils.existsFile(devControllersFile) ? [ devControllersFile ] : [];
      ...
      touch(args) {
        ...
        return this.assert("string" == typeof parameters.file, "Parameter «--file» is required as string on «DevBinaryV6.ShadowCommands.prototype.touch»"), 
        this.devbin.utils.touchFile(parameters.file, {
          uncacheInjections: parameters.uncacheInjections,
          isRoot: !0
        });
      }
    };
    static ShadowFileEvents=class {
      constructor(devbin) {
        this.devbin = devbin;
      }
    };
    ...
    static Settings=class extends ModulerV6.Settings {
      constructor(devbin) {
        super(devbin.moduler), this.devbin = devbin;
      }
      async load(forceReload = !1) {
        if (!forceReload && this.data) return this.data;
        const settingsPath = this.devbin.compiler.fullpathOf("@/dev/settings.js");
        if (!await this.devbin.utils.existsFile(settingsPath)) return this.data;
        const settingsModule = require(settingsPath), settings = "function" == typeof settingsModule ? await settingsModule.call(this.devbin) : settingsModule;
    ...
    static Tester=class {
      ...
      async runDirectory(dirInput, options = {}) {
        const {filter: filter = !1, ignore: ignore = [ "runner.js" ], injection: injection = {}, title: title = !1, filename: filename = !1} = options;
        {
          const validOptions = [ "filter", "ignore", "injection", "title", "filename" ];
          for (let prop in options) this.devbin.assert(validOptions.includes(prop), `Parameter «options» does not accept property «${prop}» on «DevBinaryV6.Tester.prototype.runDirectory»`);
        }
        const fs = require("fs"), path = require("path"), dir = this.devbin.compiler.normalizationOf(dirInput), testsType = title || path.basename(dir), testFiles = (await fs.promises.readdir(dir)).filter(file => {
          const isNotIgnored = !ignore.includes(file), passesFilter = !filter || filter(file);
          return isNotIgnored && passesFilter;
        }), ansiTool = this.devbin.compiler.constructor.ansi.colors;
        console.log(`[*] DevBinaryV6 found ${testFiles.length} tests` + (title ? ` for «${title}»` : ""));
        const errors = [], crono = this.devbin.constructor.Cronometer();
        for (let index = 0; index < testFiles.length; index++) {
          const testName = testFiles[index], testFile = `${dir}/${testName}` + (filename ? `/${filename}` : "");
          let testCallback;
          ...
          try {
            ...
            this.devbin.compiler.assert("function" == typeof _testCallback, `Test type «${testsType}» with name «${testName}» must export a callback`), 
            testCallback = _testCallback;
          } catch (error) {
            const expression = `🟣 Bad exportation on «${testsType}:${index}» named «${testName}»\n - ${error.name}: ${error.message}\n - ${error.stack}`;
            console.log(ansiTool.style("red,italic").text(ansiTool.box(expression))), errors.push({
              test: testName,
              error: error,
          ...
          if (testCallback) {
            const testCronometer = crono(`${testsType}@${index}:${testName}`).open("Started");
            try {
              await testCallback({
              ...
              const expression = `🔴 Failed «${testName}» [${testsType}:${index + 1}/${testFiles.length}]\n - ${error.name}: ${error.message}\n - ${error.stack}`;
              console.log(ansiTool.style("red,italic").text(expression)), errors.push({
                test: testName,
                error: error,
                expression: expression
              });
        ...
        if (testFiles.length) if (errors.length) {
          ...
          for (let index = 0; index < errors.length; index++) {
            ...
            console.log(ansiTool.style("magenta").text(ansiTool.box(`  - Error nº${index + 1}/${errors.length}: \n - ` + expression)));
          }
        } else console.log(ansiTool.style("greenBright,bold").text(ansiTool.box(`💎 No errors reported on «${testsType}» tests`)));
      }
    };
    ...
    async command(args = []) {
      ...
      if (Array.isArray(args)) commandParameters = this.utils.parseCliArgs(args); else {
        if ("object" != typeof args) throw new Error(`Parameter «args» must be array or object but «${typeof args}» was found instead on «DevBinary.prototype.command»`);
        commandParameters = args;
      }
      commandId = commandParameters._.join("/"), commandName = commandParameters._.join(" "), 
      ...
      Load_command_callback_from_file_or_shadowCommands: {
        let isReadable;
        try {
          await require("fs").promises.readFile(commandSubpath, "utf8"), isReadable = !0;
        } catch (error) {
          isReadable = !1;
        }
        if (!isReadable) {
          if (commandType = "hook", commandName in this.shadowCommands) {
            commandCallback = this.shadowCommands[commandName];
            break Load_command_callback_from_file_or_shadowCommands;
          }
          throw new Error(`Error of «devbin command not found» for parameters «${commandId}»`);
        }
        commandType = "file", commandCallback = require(commandSubpath);
      }
      try {
        return console.log(`[*] DevBinaryV6 executing command: ${commandName}`), await commandCallback.call(this.shadowCommands, commandParameters, this, commandType, commandSubpath);
      } catch (error) {
        throw console.error(`[!] The «devbin ${commandName}» command threw an error:`, error), 
        error;
      }
    }
    selfDispatch() {
    ...
    cloneForFile(resource, devbin = !1) {
      this.assert("string" == typeof resource, "Parameter «resource» must be string on «DevBinaryV6.prototype.cloneForFile»");
      const dirpath = require("path").dirname(this.compiler.fullpathOf(resource));
      return new this.constructor(dirpath, devbin);
    }
    ...
    constructor(basedir, parent = null) {
      ...
      this.moduler = this.compiler.moduler, this.utils = parent ? parent.utils : new this.constructor.Utils(this), 
      this.settings = new this.constructor.Settings(this), this.shadowCommands = parent ? parent.shadowCommands : new this.constructor.ShadowCommands(this), 
...
}.call(), "undefined" == typeof $devbin && ("undefined" != typeof window && (window.$devbin = mod.globalInstance), 
"undefined" != typeof global && (global.$devbin = mod.globalInstance)), "undefined" == typeof DevBinaryV6 && ("undefined" != typeof window && (window.DevBinaryV6 = mod), 
"undefined" != typeof global && (global.DevBinaryV6 = mod)), DevBinaryV6;