(function (mod) {
            ...
            return class ModulerV6 {
              ...
              static ClassSkiller = class ClassSkiller {
                ...
                static mixTraits(origin, mixable, options = {}) {
                  ...
                  const mixableDescriptors =
                    Object.getOwnPropertyDescriptors(mixable);
                  ...
                  const conflictiveNames = originKeys.filter(
                    (bkey) =>
                      mixableKeys.includes(bkey) &&
                      !overridables.includes(bkey),
                  );
                ...
                static mixInterface(
                  ...
                  for (
                    ...
                    const interfaceableProperty =
                      interfaceables[indexInterfaceable];
            ...
            try {
              ...
              if (typeof errorChecker === "function") {
                ...
                if (errorChecker.name) {
                  if (errorChecker.name !== err.name)
                    throw new this.constructor.AssertionError(
                      `Should have thrown on «${message}» but not specific «error.name»:\n  - should:  ${errorChecker.name}\n  - current: ${err.name}`,
                    );
                }
                if (errorChecker.message) {
                  if (errorChecker.message !== err.message)
                    throw new this.constructor.AssertionError(
                      `Should have thrown on «${message}» but not specific «error.message»:\n  - should:  ${errorChecker.message}\n  - current: ${err.message}`,
                    );
                }
              } else if (typeof errorChecker === "string") {
                if (errorChecker) {
                  if (errorChecker !== err.message)
                    throw new this.constructor.AssertionError(
                      `Should have thrown on «${message}» but not specific «error.message»:\n  - should:  ${errorChecker}\n  - current: ${err.message}`,
                    );
                }
          ...
          async _compileRecursively(
            ...
            Compilation_process: {
              ...
              Compile_inner_files_recursively_with_subcompiler: {
                subcompiler = this._cloneForFile(
              ...
              Beautify_and_minify: {
                if (
                  ...
                  const originalSize = this.constructor.getStringSize(
                    output.js,
                  );
              ...
              If_file_is_root: {
                if (fileParameters.isRoot) {
                  Bundle_as_CompilationResult: {
                    output = new this.constructor.CompilationResult(
                      output,
                      this,
                    );
                  }
  ...
  //*/
          ...
          _createDefaultInjectedFile(file, targetId) {
            ...
            return require("fs")
              .promises.writeFile(file, output, "utf8")
              .catch((error) => {
                console.log(
    ...
    devbin.assert(true, "Test is empty right now");
              ...
              if (!(await this.existsFile(testunitFile))) {
                ...
                muters.push(
                  await this.devbin.utils.addTouchMutedirTo(
                    require("path").dirname(testunitDir),
                  ),
                );
                ...
                muters.push(
                  await this.devbin.utils.addTouchMutedirTo(testunitDir),
                );
              }
              return {
                unitDir: testunitDir,
          ...
          async touchFile(fileBrute, optionsInput = {}) {
            ...
            try {
              ...
              Initialize_event: {
                ...
                event.currentSplittableClasses =
                  await this.getSplittableClassesFrom(filedir);
              ...
              Evento_touch: {
                ...
                Procesando_entrada: {
                  Caso_previo_1_mutedir: {
                    const mutedirPath = require("path").resolve(
                      filedir,
                      ".mutedir",
                    );
                    if (await this.devbin.compiler.files.hasFile(mutedirPath)) {
                      this.devbin.console
                        ...
                        .print(
                          `[*] DevBinaryV6 ignores touch event because .mutedir was found`,
                        );
                      break Evento_touch;
                    }
                  }
                  Caso_previo_2_splittable_class: {
                    if (!event.isSplittableClass)
                      break Caso_previo_2_splittable_class;
                    const result = await this.synchronizeSplittableClass(
                      filepath,
                      event,
                    );
                    if (result) {
                  ...
                  Caso_previo_3_splittable_method: {
                    if (event.isSplittableClass)
                      break Caso_previo_3_splittable_method;
                    if (!event.currentSplittableClasses.length)
                      break Caso_previo_3_splittable_method;
      ...
      //*/
                Triggering_onVersionate_file: {
                  ...
                  const onVersionateFile = path.join(
                    path.dirname(filepath),
                    "e.onVersionate.js",
                  );
                  try {
                    const versionDefinitions =
                      await this.triggerCallbackFromFile(onVersionateFile, {
                        file: filepath,
                        event,
                        onVersionateFile,
                      });
                    if (versionDefinitions)
                      await this.versionateEntry(versionDefinitions, {
                        file: filepath,
                        event,
                        onVersionateFile,
                      });
                    ...
                    console.log(
                      this.devbin.compiler.constructor.ansi.colors
                        .style("blackBright,italic")
                        .text(
                          `[!] DevBinaryV6 found errors loading «e.onVersionate.js» as object at «${this.devbin.moduler.rootdirOf(onVersionateFile)}» but it just ignored it`,
                        ),
                    );
                  }
                }
    ...
    //*/
          ...
          async synchronizeSplittableClass(filepath, event) {
            ...
            try {
              ...
              // ------------------------------------------------------------
              mutedir =
                await this.devbin.utils.addTouchMutedirTo(classDirectory);
              // ------------------------------------------------------------
              // 1. Leer filepath
              // ------------------------------------------------------------
          ...
          async synchronizeSplittableMethod(filepath, event) {
            ...
            try {
              ...
              // ------------------------------------------------------------
              const splittableClassFiles = event.currentSplittableClasses.map(
    ...
    //*/
          ...
          async addTouchMutedirTo(dir) {
            ...
            if (!wasMuted) {
              this.devbin.console
                .setProfile("blackBright")
                .print(
                  `[*] DevBinaryV6 mutes directory through .mutedir: ${mutedirDir}`,
                );
              await this.devbin.files.writeFile(
                mutedirFile,
                `Directory captured by process: ${process.pid}`,
              );
            } else {
              this.devbin.console
                .setProfile("blackBright")
                .print(
                  `[*] DevBinaryV6 ignores to mute directory through .mutedir: ${mutedirDir}`,
                );
            }
            return {
              cancel: () => {
                if (wasMuted) {
                  this.devbin.console
                    .setProfile("blackBright")
                    .print(
                      `[*] DevBinaryV6 ignores to unmute directory through .mutedir: ${mutedirDir}`,
                    );
                  return false;
                }
                this.devbin.console
                  .setProfile("blackBright")
                  .print(
                    `[*] DevBinaryV6 unmutes directory through .mutedir: ${mutedirDir}`,
                  );
          ...
          async getSplittableClassesFrom(dir) {
            ...
            return files.filter(
              (file) =>
                file.startsWith("splittable.") && file.endsWith(".class.js"),
            );
          }
          /**
           ...
           * @description
           */
          async versionateEntry(
            versions,
            { file: fileBrute, onVersionateFile },
          ) {
            if (typeof versions === "undefined") return -1;
            if (typeof versions === "number") return -2;
            this.devbin.moduler.assert(
              typeof versions === "object",
              `Parameter «versions» must be object, number or undefined but «${typeof versions}» was found instead on file «${onVersionateFile}» on «DevBinaryV6.Utils.prototype.versionateEntry»`,
            );
          ...
          async "bundle node module"(args, devbin) {
            ...
            Bundle_entry: {
              this.devbin.console
                .setProfile("blackBright")
                .print(
                  `[*] DevBinaryV6 bundles node module «${info.name}» from: ${this.devbin.moduler.rootdirOf(info.files.main)}`,
                );
              ...
              this.devbin.console
                .setProfile("blackBright")
                .print(
                  `[*] DevBinaryV6 bundled node module «${info.name}» successfully`,
                );
            }
            ...
            Write_outputs: {
              for (let index = 0; index < outputs.length; index++) {
                ...
                this.devbin.console
                  .setProfile("blackBright")
                  .print(
                    `[*] DevBinaryV6 exports node module «${info.name}» to: ${this.devbin.moduler.rootdirOf(output)}`,
                  );
        ...
        class Tester {
            ...
            for (let index = 0; index < testFiles.length; index++) {
              ...
              console.log(
                ansiTool
                  ...
                  .text(
                    `🟢 Start «${testName}» [${testsType}:${index + 1}/${testFiles.length}]`,
                  ),
              );
              let testCallback;
              try {
          ...
          get asserters() {
            return {
              ...
              assertDoesNotThrow: (...args) =>
                this.devbin.compiler.assertDoesNotThrow(...args),
            };
          }
        };
      /**
            ...
            return {
              add: (callback) => {
                if (typeof callback !== "function")
                  throw new Error(
                    `«Process.on(event:String).add» only accepts function but «${typeof callback}» was found instead`,
                  );
      ...
      constructor(basedir, parent = null) {
        ...
        this.settings = parent
          ? parent.settings
          : new this.constructor.Settings(this);
        /**
        ...
        this.console = parent
          ? parent.console
          : this.constructor.Console.create({ devbin: this });
      }
    };
  }.call(),
);
