        ...
        const CompilerV6 = class CompilerV6 {
          ...
          async assertThrows(callback, message, errorChecker = () => true) {
            ...
            try {
              ...
              if (err === localError) {
                throw new this.constructor.AssertionError(
                  `Should have thrown but it did not throw anything: ${err.name}: ${err.message} | ${err.stack}`,
                );
              }
              if (typeof errorChecker === "function") {
                ...
                if (errorChecker.name) {
                  if (errorChecker.name !== err.name)
                    throw new this.constructor.AssertionError(
                      `Should have thrown but not specific «error.name»:\n  - should: ${err.name}\n  - current: ${err.name}`,
                    );
                }
                if (errorChecker.message) {
                  if (errorChecker.message !== err.message)
                    throw new this.constructor.AssertionError(
                      `Should have thrown but not specific «error.message»:\n  - should: ${err.message}\n  - current: ${err.message}`,
                    );
                }
              }
              this._notifyAssertion(message);
