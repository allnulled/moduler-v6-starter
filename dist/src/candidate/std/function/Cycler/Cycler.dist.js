  ...
  return class Cycler {
    ...
    start(injection = {}) {
      if (this._async) {
        return (async () => {
          if (injection)
            Object.assign((this._injection = this._injection || {}), injection);
          if (!this._cycle)
            throw new Error(
          ...
          for (let index = 0; index < this._cycle.length; index++) {
            ...
            if (typeof step === "string") {
              if (!(step in this._steps))
                throw new Error(
                  `Required step at «${index}» to be a known step but «${step}» was found intead on «Cycler.prototype.start»`,
                );
