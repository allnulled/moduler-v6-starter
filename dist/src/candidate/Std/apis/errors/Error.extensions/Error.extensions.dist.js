  ...
  Internal_api_tools: {
    ...
    Error.tools.createFrameFilter = function (frames, ignoredFrames, ignoreds) {
      return function (frame) {
        if (
          ignoreds.includes(frame.fileName) ||
          ignoreds.includes(frame.functionName)
        )
    ...
    Error.tools.getSourceFragment = function (
      ...
      for (let i = start; i < end; i++) {
        ...
        if (i === lineIndex) {
          output.push(
            `${" ".repeat(width)} | ${" ".repeat(column - 1 - 1 - 1)}☝️`,
          ); // 👇
        }
      }
      return output.join("\n");
        ...
        return (
          output.frames
            .map((frame, index) => {
              ...
              output +=
                functionWrapper(`[function=${frame.functionName || "?"}]`) +
                `\n`;
              output +=
                fileWrapper(
                  `[file=${frame.fileName}:${frame.lineNumber || "?"}:${frame.columnNumber || "?"}]`,
                ) + `\n\n`;
