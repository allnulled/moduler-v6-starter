module.exports = async function ({ Tester }) {
  ...
  $moduler.assert(
    path2 === resolver.resolvePath([resolver.rootdir, "assets"]),
    "Can normalize rootpaths correctly (2)",
  );

  const path3 = resolver.rootpathOf(
    resolver.resolvePath([resolver.rootdir, "assets"]),
  );
  const path4 = resolver.basepathOf(
    resolver.resolvePath([resolver.rootdir, "assets"]),
  );

  $moduler.assert(
    path3 === "@/assets",
    "Can normalize rootpaths correctly (3)",
  );
  $moduler.assert(
    path4 === "./assets",
    "Can normalize basepaths correctly (4)",
  );
};
